...
from .const import STREDA_AUTHENTICATION_API_URL, CLIENT_ID, STREDA_B2C_TOKEN_URL, STREDA_DATA_API_URL, STREDA_SIGNALR_NEGOTIATE_URL
...
class StredaApiClient:
    ...
    def __init__(self, refresh_token: str, location_id: str, session: aiohttp.ClientSession, token_update_callback: callable = None):
    ...
    async def verify_token_validity(self) -> bool:
        ...
        _LOGGER.debug("Token expiry date: %s, now: %s, diff: %s", self._expiry_date, now, self._expiry_date - now)
        return now < (self._expiry_date - timedelta(hours=1)) # Consider token invalid if less than 1 hour left
        ...
        try:
            async with async_timeout.timeout(10):               
                ...
                async with self._session.post(url, json=self._id_token) as response:
                    ...
                    _LOGGER.debug("Successfully authenticated API token, new expiry: %s", self._expiry_date)
                    return True
    ...
    async def get_signalr_access_token(self) -> str:
        ...
        try:
            async with async_timeout.timeout(10):
                ...
                async with self._session.post(STREDA_SIGNALR_NEGOTIATE_URL, headers=negotiate_headers) as response:
                    response.raise_for_status()
                    data = await response.json()
    ...
    async def toggle_light(self, dock_number: int, dock_device_number: int) -> bool:
        ...
        try:           
            async with async_timeout.timeout(10):
                ...
                url = f"{STREDA_DATA_API_URL}/DeviceState/{self._location_id}/deviceState"
                payload = {"action":"ActionSwitch",
                           "actionParameters":{"switchAction":"TOGGLE"},
                           "targetDevice":{"deviceNumber":dock_device_number,
